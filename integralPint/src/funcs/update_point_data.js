...
module.exports = {
    handler: async function (ctx) {
        let token
        let version
        let Knocknock_env = ctx.env === 'prod' ?'trpc://Production.trpc.knocknock.AuthCenter.AuthCenter' :  'trpc://Development.trpc.knocknock.AuthCenter.AuthCenter'
        let adminsvrenv = ctx.env === 'prod' ? 'trpc://Production.trpc.vap_oteam.delivery_admin_svr.PaySecProxySvr' : 'trpc://Development.trpc.vap_oteam.delivery_admin_svr.PaySecProxySvr'
        const { body, cgiRequest } = ctx;
        const { result: tokenRsp } = await cgiRequest({
            url: 'trpc://Production.trpc.knocknock.AuthCenter.AuthCenter', // trpc://{北极星命名空间}.{北极星名字}
            trpc: {
                uuid: 'trpc.authcenter_2022032303', // uuid用作协议编译及socket缓存key，如变更协议，需要同步修改该uuid
        ...
        const { result } = await cgiRequest({
            url: 'trpc://Production.trpc.vap_oteam.delivery_admin_svr.PaySecProxySvr', // 正式:Production 测试:Development
            trpc: {
                uuid: 'trpc.vap_oteam.delivery_admin_svr_2022032304',
                json: require('./delivery_admin_svr.proto'),
                func: '/trpc.vap_oteam.delivery_admin_svr.PaySecProxySvr/UpdatePoint',
                req: 'UpdatePointReq',
                res: 'UpdatePointRsp'
            },
            data: {
